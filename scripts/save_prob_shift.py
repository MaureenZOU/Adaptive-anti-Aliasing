...
def validate_shift(val_loader, model, args):
    ...
    with torch.no_grad():
        ...
        for i, (input, target) in enumerate(val_loader):
            ...
            if (not os.path.exists(os.path.join(args.out_dir, folder_name))):
                os.mkdir(os.path.join(args.out_dir, folder_name))

            torch.save(view, os.path.join(args.out_dir, folder_name, str(i) + '_' + str(torch.argmax(output, dim=1)[0].item()) + '_' + str(target[0].item()) + '.da'))
