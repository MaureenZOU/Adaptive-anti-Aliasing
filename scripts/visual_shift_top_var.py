    ...
    with torch.no_grad():
        ...
        for i, ((input, target), pth) in enumerate(val_loader):
            ...
            if i % 25 == 0:
                ...
                hist_names.append(os.path.join(args.out_dir, 'visual', class_name, 'bar_' + img_name[:-5] + '_h_0_w_0.png'))
                ...
                for k in range(0, len(offset)):
                    ...
                    cur_img_name = 'a_' + img_name[:-5] + '_h_' + str(k) + '_w_0.png'
                    h_name = os.path.join(args.out_dir, 'visual', class_name, cur_img_name)
                    hist_names.append(os.path.join(args.out_dir, 'visual', class_name, 'bar_' + img_name[:-5] + '_h_' + str(k) + '_w_0.png'))
                    ...
                    cur_img_name = 'a_' + img_name[:-5] + '_h_0_w_' + str(k) + '.png'
                    w_name = os.path.join(args.out_dir, 'visual', class_name, 'a_' + img_name[:-5] + '_h_0_w_' + str(k) + '.png')
                    hist_names.append(os.path.join(args.out_dir, 'visual', class_name, 'bar_' + img_name[:-5] + '_h_0_w_' + str(k) + '.png'))
        ...
        sorted_var_top = sorted(video_pred_var.items(), key=lambda item: item[1], reverse=True)
        sorted_var_back = sorted(video_pred_var.items(), key=lambda item: item[1], reverse=False)

        for i in range(0, 100):
            vid_name, var = sorted_var_top[i]
            video_visual_pths, video_captions = video_to_root_visual_pth[vid_name]
            for j in range(0, len(video_visual_pths)):
                top_visualizer.insert(video_visual_pths[j], video_captions[j][:-4] + '_' + str(var)[0:6])

            vid_name, var = sorted_var_back[i]
            video_visual_pths, video_captions = video_to_root_visual_pth[vid_name]
            for j in range(0, len(video_visual_pths)):
                bottom_visualizer.insert(video_visual_pths[j], video_captions[j][:-4] + '_' + str(var)[0:6])
